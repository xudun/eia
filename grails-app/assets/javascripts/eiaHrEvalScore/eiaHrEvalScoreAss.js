...
function assComment(staffId){
    layui.use( 'layer', function() {
        ...
        $.ajax({
            type: 'POST',
            url: '/eia/eiaHrEvalScoreDetail/isEiaHrEvalScore?staffId='+staffId+"&jobRatingType="+jobRatingType+"&assessmentMonth="+assessmentMonth,
            success: function (data) {
                if(data== 'isNotScore'){
                    layer.msg('请先打分再评语！', {icon:7});
                    return;
                }else{
                    var pageUrl = "/eia/eiaHrEvalScoreDetail/eiaHrEvalCommentDetail?staffId="+staffId+"&jobRatingType="+jobRatingType+"&assessmentMonth="+assessmentMonth
                    layer.open({
                        type : 2,
                        title :" ",
                        maxmin: true,
