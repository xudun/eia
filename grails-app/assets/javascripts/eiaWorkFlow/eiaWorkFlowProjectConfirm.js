layui.use(['jquery', 'form', 'layer', 'laydate'], function () {
    ...
    if (nodesCode == 'XMGD'||nodesCode=='JLJS') {
        $('.canHideItem').find('.layui-form-label span').text("");
        $('.canHideItem').find('.layui-input-block input').attr("lay-verify", "");
        $('.canHideItem').remove();
    }
    if ((nodesCode != 'XMGD'||nodesCode!='JLJS') && ajaxUrl.indexOf('Halt') < 0) {
        //渲染节点审核人select选项（下个节点的节点审核人！！！！）
        $.ajax({
            ...
            success: function (res) {
                ...
                $.ajax({
                    ...
                    type: "POST",
                    data: {"eiaWorkFlowBusiId": eiaWorkFlowBusiId, "nodesCode": processUrlParams},//下个节点的
                    dataType: "json",
                    async: true,
